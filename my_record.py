...
class Records:
    ...
    def display_books(self):
        ...
        textbooks = [b for b in self.books if b.book_type == 'Textbook']
        fictions = [b for b in self.books if b.book_type == 'Fiction']

        textbooks.sort(key=lambda x: x.name)
        fictions.sort(key=lambda x: x.name)

        print("TEXTBOOKS")
        self._print_book_table(textbooks)

        print("\nFICTIONS")
        self._print_book_table(fictions)

    def _print_book_table(self, books):
        col_widths = {
            'book_id': 10,
            'name': 20,
            'book_type': 10,
